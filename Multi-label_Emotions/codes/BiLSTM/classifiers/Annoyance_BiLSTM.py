...
def mainModel(path, train_set, train_label, dev_set, dev_label):
    ...
    model = Sequential()
    model.add(Bidirectional(LSTM(num_neurons, return_sequences=True), input_shape=(maxlen, embedding_size)))
    ...
    with open(path + 'result_data\\Positive_BiLSTM.json', 'w') as j:
        j.write(model_structure)

    model.save_weights(path + 'result_data\\Positive_BiLSTM_weights.h5')


def testModel(path, test_set, test_label):
    ##reload model
    with open(path + 'result_data\\Positive_BiLSTM.json', 'r') as j:
        json_str = j.read()
    model = model_from_json(json_str)

    model.load_weights(path + 'result_data\\Positive_BiLSTM_weights.h5')
...
if __name__ == '__main__':
    ...
    train_set, train_label, dev_set, dev_label, test_set, test_label = preLoad(path, 12)
