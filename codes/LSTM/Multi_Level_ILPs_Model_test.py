...
def Multi_Level_Model(path):
    ...
    test_set = test_set[5389:5391]
    test_label = test_label[5389:5391]
...
#---------------------------Positive--------------------------------------------------------------------------------
            if first_level == 'Positive_LSTM' and f_label == 1:
                ...
                for second_level in ['Joy_ekman_LSTM']:
                    ...
                    P2.append(s_pro)
                    s_label = (s_pro > s_cutoff).astype('int')
                ...
                if empty_flag:
                    choosed = ILP(P2)
                    for e_j in range(len(choosed)):
                        P3 = []
                        isEmptyFlag = True
                        if e_j == 0 and choosed[e_j] == 1:
                            ...
                            for third_level in joy_ekmans:
                                ...
                                print('{}: predicted-probability:{} predicted-label:{}'.format(third_level, t_pro,
                                                                                               t_label))
            ...
            if first_level == 'Negative_LSTM' and f_label == 1:
                ...
                for second_level in ['Sadness_ekman_LSTM', 'Fear_ekman_LSTM', 'Disgust_ekman_LSTM', 'Anger_ekman_LSTM']:
                    ...
                    if s_label == 1 and second_level == 'Sadness_ekman_LSTM':
                        empty_flag = False
                        sadness_ekmans = ["Sadness_LSTM", "Disappointment_LSTM", "Embarrassment_LSTM", "Grief_LSTM",
                                          "Remorse_LSTM"]
