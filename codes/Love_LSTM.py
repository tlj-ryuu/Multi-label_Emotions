...
def mainModel(path, train_set, train_label, dev_set, dev_label):
    ...
    with open(path + 'result_data\\Love_LSTM.json', 'w') as j:
        j.write(model_structure)

    model.save_weights(path + 'result_data\\Love_LSTM_weights.h5')


def testModel(path, test_set, test_label):
    ##reload model
    with open(path + 'result_data\\Love_LSTM.json', 'r') as j:
        json_str = j.read()
    model = model_from_json(json_str)

    model.load_weights(path + 'result_data\\Love_LSTM_weights.h5')
...
if __name__ == '__main__':
    path = 'D:\\计算机毕业设计\\'
    train_set, train_label, dev_set, dev_label, test_set, test_label = preLoad(path, 27)
